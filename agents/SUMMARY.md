...
- **Per-harmonic uniform suffix for ANY harmonic set (§7.7.9)** — uniform
  layers never couple different harmonics, so the all-uniform suffix's S-matrix
  is 2×2-block-diagonal per harmonic, built by an O(nG·N) per-harmonic
  recursion instead of dense star steps or binary exponentiation. Full-2D
  `rt_solve`: 1354→369 ms (nG=201), 3268→964 ms (nG=301), R bit-identical.
  (The earlier periodic binary exponentiation — `period^(R-1)` + the last
  period's real boundary interface via `redheffer_cascade`, ~4e-15 identical —
  is superseded by this. The naive `S^R` doubling was wrong: it ends with a
  phantom reference layer that mismatches the substrate boundary.)
...
**Aug 2026 round 3 — per-harmonic uniform suffix for full 2D (§7.7.9):**
full-2D EUV absorber `rt_solve` 1354→369 ms (nG=201) and 3268→964 ms
(nG=301) — ~3.5×, growing with nG — total ~2.2×, R bit-identical.
...
~4e-4 R error). The later fix exponentiates `period^(R-1)` and cascades with
the last period's real boundary interface via the exact Redheffer cascade
...
*Superseded (Aug 2026) by §7.7.9*: the per-harmonic uniform suffix builds the
all-uniform S-matrix for any harmonic set in O(nG·N), with no periodicity
assumption. Kept here as the earlier approach; its overlapping-cascade formula
(two S-blocks sharing boundary layer $m$) is still used by the per-harmonic path.
...
#### 7.7.9 Per-harmonic uniform suffix for arbitrary harmonic sets

The quasi-1D diagonal-core tricks (§7.7.5–7.7.7) and the periodic binary
exponentiation (§7.7.4) are all special cases of a general fact: **uniform
isotropic layers never couple different diffraction orders.** Under the
$(E_{x,h},\,E_{y,h})$ interleaving, every uniform-layer operator is
block-diagonal with $2\times2$ blocks per harmonic — for **any** harmonic set
and arbitrary $(k_{x,h}, k_{y,h})$:
$$\mathsf{kp}^{(h)}=\omega^2 I_2-\frac{1}{\varepsilon}
\begin{bmatrix}k_{y,h}^2 & -k_{x,h}k_{y,h}\\
-k_{x,h}k_{y,h} & k_{x,h}^2\end{bmatrix},\qquad
\mathbf{q}_h\ \text{doubled},\qquad \phi_h=I_2,$$
since `kp = ω²I − (1/ε)·Jk·Jkᵀ` with `Jk = [−diag(ky); diag(kx)]` is exactly
block-diagonal in this ordering.

Consequently the S-matrix of any all-uniform range is $2\times2$-block-diagonal,
built by a **per-harmonic $2\times2$ Redheffer recursion** in
$O(n_G\cdot N_{\mathrm{layers}})$ instead of dense
$O\big((2n_G)^3 N_{\mathrm{layers}}\big)$ star steps. `GetSMatrix` splits the
stack at the start $m$ of the trailing uniform suffix: prefix
$S(\mathrm{indi},m)$ (patterned layers + boundary uniform layer) via the general
dense step, suffix $S(m,\mathrm{indj})$ via the per-harmonic recursion, joined by
the overlapping cascade of §7.7.4 ($L$, $R$ share layer $m$). This subsumes the
periodic binary exponentiation (§7.7.4) — the suffix is now
$O(n_G\,N_{\mathrm{layers}})$ for periodic *and* non-periodic stacks — and
reduces to the scalar TE/TM recursion (§7.7.7) when the blocks are diagonal
(ky₀=0, 1D set).

**Exact**: full-2D R is bit-identical to the previous dense/binary-exponentiated
path (0.578759 / 0.579138 / 0.579249 at requested nG = 97 / 201 / 301, EUV
absorber on the Mo/Si mirror). Measured full-2D `rt_solve`: 1354→369 ms
(nG=201) and 3268→964 ms (nG=301) — ~3.5×, growing with nG since the suffix
cost is now linear; total wall-clock ~2.2×. The remaining dense cost is the
patterned $2n_G\times2n_G$ `zgeev` plus the few prefix/cascade operations.

---

## 8. Known Gaps / Notes

- **`RT_Solve(byorder=true)`** — per-order `R_per_order`/`T_per_order` arrays
