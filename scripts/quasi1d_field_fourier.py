#!/usr/bin/env python3
"""Compare Solve_FieldFourier: grcwa (original) vs cpprcwa (nanobind).

Runs the same quasi-1D EUV line-grating geometry at oblique incidence as the
C++ example ex_quasi1d_absorber (TaN bar on the full 40x Mo/Si multilayer
mirror) through the original grcwa library and through the cpprcwa nanobind
bindings in one process, then compares the Fourier-space fields returned by
Solve_FieldFourier element-wise (same harmonic set G, same ordering) at
several layers and z-offsets.

The physical field in the harmonic basis is unique and independent of the
internal eigenvector ordering/phase of the two libraries, so the ex/ey/ez/
hx/hy/hz coefficient vectors must agree to machine precision.

Usage:
    python3 scripts/quasi1d_field_fourier.py [nG_req] [Nx] [Ny]
...
def main():
    ...
    # Harmonic sets must be identical (same nG); the ordering may differ
    # (degenerate-shell tie-break in the sort), so reorder cpprcwa fields to
    # grcwa's G ordering before comparing.
    Gg = np.asarray(g.G)
    Gc = np.asarray(c.G)
    if Gg.shape != Gc.shape or not np.array_equal(np.sort(Gg, axis=0),
        ...
        return 1
    perm = [int(np.where((Gc == Gg[j]).all(1))[0][0]) for j in range(len(Gg))]

    # Test layers: incident vacuum, TaN grating, Ru cap, a deep Mo layer in
    # the middle of the 40x multilayer, and the Si substrate.
    deep_mo = 2 * (nb // 2) + 1         # Mo layer at index 2k+1, k = nb//2
    layers = [0, 1, 2, deep_mo, g.Layer_N - 1]
    ...
    for l in layers:
        for z in zs:
            fg = np.array(g.Solve_FieldFourier(l, z)[0])
            fc = np.array(c.Solve_FieldFourier(l, z)[0])
            # fg, fc: (2, 3, nG) → flatten to (6, nG) and reorder cpprcwa.
            fg = fg.reshape(6, -1)
            fc = fc.reshape(6, -1)[:, perm]
